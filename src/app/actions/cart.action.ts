...
import { COOKIE_CART_ID, TAGS } from "@/constants";

/*export const getCart = unstable_cache(
  async (cartId: string) => storefrontClient.getCart(cartId),
  [TAGS.CART],
  { revalidate: 60 * 15, tags: [TAGS.CART] },
);*/
...
export async function addCartItem(prevState: any, variantId: string) {
  ...
  if (
    !itemAvailability ||
    itemAvailability.inCartQuantity >= itemAvailability.inStockQuantity
  )
    return {
      ok: false,
  ...
  /*  await storefrontClient.createCartItem(cartId!, [
...
export async function getItemAvailability(
  ...
  if (!cartId || !variantId)
    return { inCartQuantity: 0, inStockQuantity: Infinity };

  /*
  const cart = await storefrontClient.getCart(cartId);
*/
  /*  const cartItem = cart?.items?.find(
    (item: { merchandise: { id: string } }) =>
...
export async function updateItemQuantity(
  prevState: any,
  payload: { itemId: string; variantId: string; quantity: number },
) {
  const cartId = cookies().get(COOKIE_CART_ID)?.value;
...
*/
  ...
  /*
  await storefrontClient.updateCartItem(cartId, [
