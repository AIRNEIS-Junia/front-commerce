...
describe("FilterBuilder", () => {
  test("should create a basic condition", () => {
    const filter = new FilterBuilder()
      .where("genres", ComparisonOperators.Equal, "horror")
      .build();
  ...
  test("should combine conditions with AND/OR logical operators", () => {
    const filter = new FilterBuilder()
      ...
      .build();
    const filterOr = new FilterBuilder()
      .where("genres", ComparisonOperators.Equal, "horror")
      .or()
      .where("genres", ComparisonOperators.Equal, "comedy")
      .build();
  ...
  test("should handle complex grouping with logical operators", () => {
    ...
    expect(filter).toBe(
      '(genres = "horror" OR genres = "comedy") AND rating.users > 85',
    );
  });

  test("should create an inequality condition", () => {
    const filter = new FilterBuilder()
      .where("genres", ComparisonOperators.NotEqual, "action")
      .build();
  ...
  test('should combine comparison conditions using ">= AND <" operators', () => {
    const filter = new FilterBuilder()
      .where("rating.users", ComparisonOperators.GreaterThanOrEqual, 80)
      .and()
      .where("rating.users", ComparisonOperators.LessThan, 90)
      .build();
  ...
  test('should handle reverse "NOT EXISTS" logic', () => {
    const filter = new FilterBuilder()
      .not()
      .where("release_date", SpecialOperators.Exists)
      .build();
  ...
  test("should combine multiple conditions and groups", () => {
    ...
    expect(filter).toBe(
      '(genres = "horror" OR genres = "comedy") AND director = "Jordan Peele"',
    );
  });
  ...
  test("should handle complex condition with OR and NOT operators", () => {
    ...
    expect(filter).toBe(
      '(genres = "comedy" OR genres = "horror") AND NOT director = "Jordan Peele"',
    );
  });
  ...
  test("should handle mixed array and string syntax for complex condition", () => {
    ...
    expect(filter).toBe(
      '(genres IN ["comedy", "horror"]) AND NOT director = "Jordan Peele"',
    );
  });
  ...
  test("should not start with a logical operator", () => {
    const query = new FilterBuilder()
      .and()
      .where("field", ComparisonOperators.Equal, "value")
      .build();
  ...
  test("should handle complex expressions without starting or ending with a logical operator", () => {
    const query = new FilterBuilder()
      ...
      .group((builder) => {
        builder
          .where("field2", ComparisonOperators.GreaterThan, 10)
          .or()
          .where("field3", ComparisonOperators.LessThan, 20);
      })
      .or()
      .build();
