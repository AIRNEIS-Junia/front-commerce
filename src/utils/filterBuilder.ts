...
export class FilterBuilder {
  ...
  between(attribute: string, min: number, max: number): FilterBuilder {
    this.expression.push(
      `${attribute} ${ComparisonOperators.GreaterThanOrEqual} ${min} AND ${attribute} ${ComparisonOperators.LessThanOrEqual} ${max}`,
    );
    return this;
  }
  ...
  where(
    attribute: string,
    operator: ComparisonOperators | SpecialOperators,
    value?: Value,
  ): FilterBuilder {
    if (Array.isArray(value)) {
      ...
      typeof value === "string" &&
      operator !== ComparisonOperators.To
    ) {
      value = `"${value}"`;
    }
  ...
  to(attribute: string, min: number, max: number): FilterBuilder {
    this.expression.push(
      `${attribute} ${min} ${ComparisonOperators.To} ${max}`,
    );
    return this;
  }
  ...
  build(): string {
    const isConnectingOperatorFirst =
      this.expression.length > 0 &&
      this.isConnectingOperator(this.expression[0]);
    const isConnectingOperatorLast =
      this.expression.length > 0 &&
      this.isConnectingOperator(this.expression[this.expression.length - 1]);
